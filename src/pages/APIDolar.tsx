...
export default function APIDolarPage() {
  ...
  const [configs, setConfigs] = useState<{
    ...
    const savedConfigs = localStorage.getItem('api-configs-dolar');
  ...
  const handleSave = (section: string, newConfig: APIConfig) => {
    ...
    localStorage.setItem('api-configs-dolar', JSON.stringify(updatedConfigs));
    toast({
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-trader-green">Configuração de APIs - Dólar</h1>
          <Button
            variant="outline"
            className="gap-2"
